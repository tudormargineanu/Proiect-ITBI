...
do
  ...
  #adauga proprietatile fisierului vechi in variabila original
    ...
    for i in $original
    do
      if [[ $i != ${cuvinte_rezultat[$c]} && $c < 5 ]]; then
...
do
  ...
  if [[ $vftext == 1 ]]; then
    awk -v n=$lin 'NR==n {print $0}' $1 >> vf_typescript_original.txt
  fi
...
do
  ...
  if [[ $vftext == 1 ]]; then
    awk -v n=$lin 'NR==n {print $0}' $2 >> vf_typescript_modificat.txt
  fi
